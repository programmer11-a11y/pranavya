  ...
  jQuery(document).ready(function () {
    ...
    function applyScrolledState() {
      ...
      if (useBlack) {
        ...
        // TEXT + ICONS → BLACK
        jQuery(".all-header > .container .menu_link_text, .all-header > .container .site-search__open, .all-header > .container .nav-icon")
          .removeClass("text-white")
...
============================================================ */
  ...
  jQuery(document).ready(function ($) {
    ...
    $(document).on("click", ".schedule-tab", function () {
      ...
      // reset all tabs
      $btns.removeClass("bg-primary text-white")
...
(function ($) {
  $(document).on("submit.newsletterValidation", "#newsletter-form", function (e) {
    e.preventDefault();
...
$(function () {
  ...
  const thumbWidth = 140 + 16; // width + gap
  let current = 0;

  function updateThumbPosition(index) {
    const visible = parseFloat(
      getComputedStyle(document.documentElement)
        .getPropertyValue("--thumb-visible")
    );

    // shift logic
    let offsetIndex = index - Math.floor(visible - 1);
    if (offsetIndex < 0) offsetIndex = 0;
    if (offsetIndex > total - visible)
      offsetIndex = total - visible;

    $("#thumbTrack").css(
      "transform",
      `translateX(-${offsetIndex * thumbWidth}px)`
    );
  }
  ...
  jQuery(document).ready(function () {
    ...
    function applyScrolledState() {
      const scrolled = jQuery(window).scrollTop() > 0;
      const megaOpen = jQuery(".mega-panel:visible").length > 0;
      const sidebarOpen = jQuery("#mainmenu").hasClass("active");
...
============================================================ */
  ...
  // ===== Header Js ===== //
  // Removed duplicate scroll handler, handled by applyScrolledState
  jQuery(".all-header").hover(
    function () {
      jQuery(".all-header").addClass("not_scrolled_hover");
      jQuery(".all-header:not(.scrolled) .site-logo img").attr(
...
$(function () {
  ...
  // ===================== SHOW SLIDE =====================
  function showSlide(index) {
    if (index === current) return; // ⛔ prevent useless repaint
    ...
    $("#slideTitle").text($slide.data("title"));
    $("#slideDesc").text($slide.data("desc"));
